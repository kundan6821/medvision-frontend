...
function handleFileSelect(file) {
    ...
    uploadArea.innerHTML = `
        <div class="upload-content">
            <i class="fas fa-check-circle upload-icon" style="color: #E30613;"></i>
...
async function analyzeImage() {
    ...
    searchResults.innerHTML = '<div class="loading"><i class="fas fa-spinner fa-spin"></i> Optimizing image size...</div>';
    ...
    try {
        ...
        console.warn('Image compression failed, proceeding with original file:', compressErr.message);
    }
...
function displayResults(data, isImageSearch = false) {
    ...
    if (!isImageSearch) {
        html += '<div style="padding: 15px; border-bottom: 1px solid rgba(227, 6, 19, 0.1); display: flex; justify-content: space-between; align-items: center;">';
        html += '<h3 style="margin: 0; color: #E30613; font-size: 16px;"><i class="fas fa-clipboard-list"></i> Search Results</h3>';
        html += '<button onclick="resetSearch()" style="padding: 8px 16px; background: rgba(227, 6, 19,0.2); border: 1px solid rgba(227, 6, 19,0.4); color: #E30613; border-radius: 8px; cursor: pointer; font-size: 12px; transition: all 0.3s;" onmouseover="this.style.background=\'rgba(227, 6, 19,0.3)\'" onmouseout="this.style.background=\'rgba(227, 6, 19,0.2)\'">';
    ...
    if (isImageSearch && ocrInfo) {
        html += '<div class="result-section" style="background: rgba(227, 6, 19, 0.1); border: 1px solid rgba(227, 6, 19, 0.3);">';
        ...
        if (ocrInfo.confidence < 80) {
            ...
            html += `<input type="text" id="correctedName" value="${ocrInfo.detected_medicine}" style="flex: 1; padding: 8px; background: rgba(255,255,255,0.1); border: 1px solid rgba(227, 6, 19,0.3); border-radius: 5px; color: white;">`;
            html += '<button onclick="searchCorrectedName()" style="padding: 8px 15px; background: linear-gradient(135deg, #C30000 0%, #E30613 100%); border: none; color: white; border-radius: 5px; cursor: pointer;">Search</button>';
            html += '</div>';
            html += '</div>';
        }
    ...
    if (Object.keys(medicineInfo).length > 0) {
        ...
        if (medicineInfo.common_uses) {
            html += `<p style="margin-top: 15px;"><strong style="color: #E30613;">Common Uses:</strong> ${medicineInfo.common_uses}</p>`;
        }
        if (medicineInfo.dosage_guidance) {
            html += `<p style="margin-top: 10px;"><strong style="color: #E30613;">Dosage Guidance:</strong> ${medicineInfo.dosage_guidance}</p>`;
        }
    ...
    if (alternatives.length > 0) {
        ...
        html += '<p style="color: #E30613; margin-bottom: 15px; font-size: 14px;">These medicines have the same active ingredient(s) and can be used as substitutes:</p>';
        ...
        alternatives.forEach(alt => {
            ...
            // Comparison info
            html += `<span style="display: block; font-size: 11px; color: #E30613; margin-top: 5px;">`;
...
// API URL (Node Server)
const AUTH_API = (window.BACKEND_URL || 'https://medvision-backend-egoh.onrender.com').replace(/\/$/, '') + '/api';
...
function renderReminderList(reminders) {
    ...
    list.innerHTML = reminders.map(r => `
        <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(227, 6, 19,0.06); border:1px solid rgba(227, 6, 19,0.2); border-radius:12px; padding:14px 18px; margin-bottom:10px;">
            <div>
                <span style="color:#fff; font-weight:600; font-size:15px;"><i class="fas fa-pills" style="color:#E30613; margin-right:8px;"></i>${r.medicine}</span>
                <span style="display:block; color:#E30613; font-size:13px; margin-top:4px;"><i class="fas fa-clock" style="margin-right:6px;"></i>${r.time}</span>
            </div>
    ...
    banner.innerHTML = `
        <div style="display:flex; align-items:center; gap:15px;">
            ...
            <div>
                ...
                <div style="font-size:14px; color:#E30613; margin-top:3px;">Time to take: <strong>${medicine}</strong></div>
            </div>
        </div>
...
// ---- Interactive Safety Sandbox Logic ----
const sandboxCheckBtn = document.getElementById('sandboxCheckBtn');
if (sandboxCheckBtn) {
    sandboxCheckBtn.addEventListener('click', () => {
        const allergySelect = document.getElementById('sandboxAllergy');
        const drugInput = document.getElementById('sandboxDrugInput');
        const resultContainer = document.getElementById('sandboxResult');
        ...
        resultContainer.innerHTML = `
            <div class="sandbox-result-card" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #fff;">
                <div class="sandbox-result-title"><i class="fas fa-spinner fa-spin" style="color:#E30613;"></i> Running simulated clash analysis...</div>
            </div>
        `;
        ...
        setTimeout(() => {
            ...
            // Simple simulated clash rules
            if (allergy === 'penicillin') {
                if (/\b(ceflox|cipro|penicillin|amoxicillin|ampicillin|augmentin|cefixime|ceftriaxone|cefadroxil|cephalexin|keflex|mox|clav)\b/i.test(drugLower) || 
                    drugLower.includes('cef') || drugLower.includes('cillin') || drugLower.includes('pen') || drugLower.includes('cipro') || drugLower.includes('ceflox')) {
                    clash = true;
                    explanation = `⚠️ <strong>Clash Detected!</strong> "${drug}" belongs to or cross-reacts with the Beta-lactam family (Penicillins/Cephalosporins). Since you are allergic to Penicillin, taking this drug could cause a severe hypersensitivity reaction.`;
                }
            } else if (allergy === 'aspirin') {
                if (/\b(aspirin|disprin|ecotrin|ibuprofen|advil|motrin|naproxen|aleve|combiflam|flexon|brufen|combipara|diclofenac|voveran)\b/i.test(drugLower) || 
                    drugLower.includes('aspirin') || drugLower.includes('ibu') || drugLower.includes('fen') || drugLower.includes('diclo')) {
                    clash = true;
                    explanation = `⚠️ <strong>Clash Detected!</strong> "${drug}" contains active NSAID ingredients (like Aspirin or Ibuprofen). Your allergy profile indicates NSAID sensitivity, which may trigger asthma attacks, hives, or gastrointestinal discomfort.`;
                }
            } else if (allergy === 'sulfa') {
                if (/\b(bactrim|septra|sulfamethoxazole|sulfasalazine|sulfa|dapsone)\b/i.test(drugLower) || drugLower.includes('sulfa')) {
                    clash = true;
                    explanation = `⚠️ <strong>Clash Detected!</strong> "${drug}" is a sulfonamide (sulfa) antibiotic. Ingesting this with a documented sulfa allergy can lead to severe skin rashes (e.g. Stevens-Johnson syndrome) or drug fever.`;
                }
            }

            if (clash) {
                resultContainer.innerHTML = `
                    <div class="sandbox-result-card result-clash">
                        <div class="sandbox-result-title color-clash">
                            <i class="fas fa-circle-exclamation"></i> Conflict Found: High Risk
                        </div>
                        <div class="sandbox-result-body">
                            <p>${explanation}</p>
                            <p style="margin-top: 10px; font-size:12px; color:rgba(231,76,60,0.8);"><em>*Note: This is a demo sandboxed evaluation. Register for a full-scale AI lookup check.</em></p>
                        </div>
                    </div>
                `;
            } else {
                resultContainer.innerHTML = `
                    <div class="sandbox-result-card result-safe">
                        <div class="sandbox-result-title color-safe">
                            <i class="fas fa-circle-check"></i> No Conflict Detected: Safe
                        </div>
                        <div class="sandbox-result-body">
                            <p>✅ <strong>Safe Evaluation:</strong> No overlapping ingredients or cross-reacting chemical families were found between "${drug}" and your selected allergen profile (${allergy === 'none' ? 'None' : allergy.toUpperCase()}).</p>
                            <p style="margin-top: 10px; font-size:12px; color:rgba(46,204,113,0.8);"><em>*Disclaimer: This is a sandboxed mockup. Always consult a healthcare professional.</em></p>
                        </div>
                    </div>
                `;
            }
        }, 800); // Small delay to simulate scanning and calculations
    });
}
